...
function jhp_post_link( $permalink, $post ) {	
	if( $post->post_type !== 'jh-portfolio' )
		return $permalink;
	$link = trailingslashit(get_bloginfo('url')) . trailingslashit(get_option('jhp_url_base', 'portfolio')) . array_shift(wp_get_object_terms( $post->ID, 'jh-portfolio-category' ))->slug . '/' . $post->post_name . '/';
	return $link;
...
function jhp_add_link_to_wp_list_pages( $data, $args ) {
	$li = '<li class="page_item"><a href="' . trailingslashit(get_bloginfo('url')) . get_option('jhp_url_base', 'portfolio') . '">' . get_option('jhp_title', 'Portfolio') . '</a></li>';

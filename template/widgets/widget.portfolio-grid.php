...
class WP_Widget_hmp_Portfolio_Grid extends WP_Widget {
	// Constructor
	function WP_Widget_hmp_Portfolio_Grid() {
		$widget_ops = array( 'description' => __( 'Show a grid of all portfolio entries with thumbnails' ) );
		$this->WP_Widget( 'hmp_portfolio_grid', __( 'HMP Grid' ), $widget_ops );
	}
	...
	function widget( $args, $instance ) {	
		...
		?>
		<div id="hmp-portfolio-grid">
			<?php foreach( get_terms( 'hmp-entry-category', array( 'parent' => 0 ) ) as $cat ) :
			 ?>
				<?php $hmp_portfolio = new WP_Query( array( 'taxonomy' => 'hmp-entry-category', 'term' => $cat->slug, 'showposts' => -1, 'orderby' => $sort_by, 'post_type' => 'hmp-entry') ); ?>
...
add_action('widgets_init', 'widget_hmp_portfolio_grid');
function widget_hmp_portfolio_grid() {
	register_widget( 'WP_Widget_hmp_Portfolio_Grid' );
}
?>